...
import { getMashup } from './get-mashup';
...
class Encryption {
	...
	createKeys = async (userId: string, server: string, autoGenerate = false) => {
		// Generate new keys
		...
		if (autoGenerate) {
			...
			log(`autogenerated ${mashup} for user ${userId}`); // TODO: sensitive - remove in production
			password = mashup;
		} else {
			// Create a password to encode the private key
			password = await this.createRandomPassword(server);
		}
