...
import { StyleSheet, Text, View, TextInput as RNTextInput, Alert } from 'react-native';
...
class E2EEncryptionSecurityView extends React.Component<IE2EEncryptionSecurityViewProps, IE2EEncryptionSecurityViewState> {
	...
	showAutoGeneratedPassword = () => {
		const { server, user } = this.props;
		const userId = user.id;
		const mashup = getMashup(server, userId);
		Alert.alert(I18n.t('Your_password_is'), mashup);
	};
	...
	renderShowAutoGeneratedPassword = () => (
		<>
			<List.Section>
				<Button
					onPress={this.showAutoGeneratedPassword}
					title={I18n.t('Show_Auto_Generated_Password')}
					style={styles.changePasswordButton}
					testID='e2e-encryption-security-show-auto-generated-password'
				/>
			</List.Section>
		</>
	);
	...
	render() {
		const { theme, autoGeneratedPassword } = this.props;
		return (
				...
				<List.Container>
					<View style={styles.container}>
						{autoGeneratedPassword
							? this.renderShowAutoGeneratedPassword()
...
const mapStateToProps = (state: IApplicationState) => ({
	...
	autoGeneratedPassword: state.settings.Peers_AutoGenerateE2EKeyPassword as boolean
});
